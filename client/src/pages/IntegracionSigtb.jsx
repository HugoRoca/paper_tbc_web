...
import { ArrowLeft, Save, X, Search, FileText } from 'lucide-react'
import toast from 'react-hot-toast'
import { toInputDate } from '../utils/date'

const IntegracionSigtb = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data) => integracionService.consultarSIGTB(data),
    onSuccess: (response) => {
    ...
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Error al consultar SIGTB')
    },
  })
  ...
  const validate = () => {
    const newErrors = {}
    if (!formData.dni && !formData.codigo_caso) {
      newErrors.dni = 'Ingrese DNI o código de caso'
      newErrors.codigo_caso = 'Ingrese DNI o código de caso'
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.codigo_caso) dataToSend.codigo_caso = formData.codigo_caso.trim()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Consultar SIGTB</h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Código de Caso</label>
              <input
                type="text"
                name="codigo_caso"
...
export default IntegracionSigtb
