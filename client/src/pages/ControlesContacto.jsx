...
const ControlesContacto = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['controles-contacto', page, limit, filters],
    queryFn: () => {
      const params = {
        page,
        limit,
      ...
      return controlContactoService.list(params)
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => controlContactoService.delete(id),
    onSuccess: () => {
  ...
  const controles = data?.data || []
  const pagination = data?.pagination || { page: 1, total: 0, totalPages: 1 }

  const filteredControles = useMemo(() => {
    if (!searchTerm) return controles
    const term = searchTerm.toLowerCase()
    return controles.filter((control) => {
      const contacto = control.contacto
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center bg-red-50 border border-red-200 rounded-lg p-6 max-w-md">
          <p className="text-red-600">Error al cargar controles: {error.message}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Controles de Contacto</h2>
          <p className="text-gray-600 mt-1">Seguimiento programado y realizado</p>
        </div>
        <Link
          to="/controles-contacto/nuevo"
        ...
        <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-700">
              Total: {pagination.total} control{pagination.total !== 1 ? 'es' : ''}
            </span>
          </div>
        </div>
                ...
                {filteredControles.map((control) => (
                  <tr key={control.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500">{control.contacto?.dni || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">#{control.numero_control}</div>
                      <div className="text-xs text-gray-500">{control.tipo_control}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-500">
                        {formatDateLocal(control.fecha_programada)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-500">
                        {control.establecimiento?.nombre || '-'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link
                          to={`/controles-contacto/${control.id}/editar`}
...
export default ControlesContacto
